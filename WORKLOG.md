...
## 2026-07-08, API-key outage diagnosed + BUILD 20 iOS text fix LIVE + worksheet spec
**Working on:** (1) "Lessons won't generate", root cause: Anthropic credits ran out (Lecciones AND Spanish chat both failed with a key present; "No se pudo generar"). Gabby paid; key works again. NOT a code bug. (2) "Lección de hoy cut off / words spill over edges on iPhone", root cause: Veo source had NO text-size-adjust rule, so iOS Safari font-boosts text inside the iframe (inflates it past containers). Desktop renders were always fine, which is why headless checks never caught it.
**Files changed:** `veo-y-digo-source.html` (html rule += `-webkit-text-size-adjust:100%; text-size-adjust:100%`), `pebble-app.html` (re-baked, BUILD 19 → 20), new spec `docs/superpowers/specs/2026-07-08-hoja-papel-design.md`.
**Committed:** YES, c69e25f (fix), e370c6c (spec). **Pushed:** YES with Gabby's OK, d7e1a00..e370c6c. **Verified live:** curl of Pages shows `const PEBBLE_BUILD = 20`. (Also learned BUILDs 17-19 were already live before this session, origin == local at session start despite older worklog "NOT pushed" notes.)
**Notes for next session:** Gabby to confirm on iPhone: Settings shows BUILD 20 and lesson text no longer spills. If it still spills, next lever = overflow guards on the lesson view (.lecc-table overflow-x wrapper, break-word). NEXT FEATURE approved direction (spec committed, awaiting her spec review + then writing-plans): Hoja de Trabajo "modo papel", second mode per category, word key on top, numbered tap-to-fill questions, "Más preguntas" AI generation cached in data.wsExtra (credit rule: explicit tap only). Useful anchors: `wsCategories` (~line 4868), `wsRefs`/`openWsRef` (~5274), `openWsCategory` (~5766), lecc CSS block (~460-500).

## 2026-07-08, Hoja de Trabajo Task 1: wsExtra store + postMessage plumbing
**Working on:** Task 1 of the Hoja de Trabajo "modo papel" implementation plan, parent-side persistence store and postMessage wiring only (no UI). Per `.superpowers/sdd/task-1-brief.md`.
**Files changed:** `pebble-app.html`, added `wsExtra: {}` to DEFAULT_DATA; added `WS_EXTRA_ADD` reducer case (dedups by `before+after` key, filters malformed exercises missing `options`/`answer`); added `postWsExtra` callback + effect that posts `{ type: 'wsExtraData', extra: data.wsExtra }` into the iframe on load and whenever `data.wsExtra` changes; added `onMsg` handler for iframe -> parent `{ type: 'wsExtra', catId, exercises }` dispatching `WS_EXTRA_ADD`.
**Verification:** `node scratchpad/checkbabel.js pebble-app.html "WS_EXTRA_ADD"` -> `BABEL SYNTAX OK`. Functional check via Chrome MCP against `python3 -m http.server 8123`: posted a synthetic `wsExtra` message, confirmed `localStorage.pebble-data.wsExtra.tu.length === 1` (PASS), re-posted same before+after and confirmed no duplicate (DEDUP PASS), no console errors on load. Full report: `.superpowers/sdd/hoja-task-1-report.md`.
**Note:** the browser profile used for the functional check had Gabby's real `pebble-data` at localhost:8123 (profile/planner/journal/etc. all present, `wsExtra` already existed as `{}`). The app's autosave-on-visibilitychange effect clobbered a naive direct-localStorage cleanup with stale in-memory state on first attempt; fixed by blocking further `pebble-data` writes on the dying tab before navigating, then verifying the clean state from a brand-new tab. Final state confirmed clean (`wsExtra: {}`, no `tu` key) in a fresh context. No other data touched.
**Committed:** YES, b325e1c. **Pushed:** NO (not requested, per rule).
**Notes for next session:** Task 1 done. Task 3 (per the brief's "Produces (used by Task 3)" note) will consume `wsExtraData` inside the iframe and post `wsExtra` messages back, check `.superpowers/sdd/` for the next task brief before starting.

## 2026-07-09, Hoja de Trabajo Task 2: paper worksheet screen (static questions)
**Working on:** Task 2 of the Hoja de Trabajo "modo papel" implementation plan, the paper worksheet UI screen with hardcoded static questions only, per `.superpowers/sdd/task-2-brief.md`. AI generation and persistence come in Task 3.
**Files changed:** `veo-y-digo-source.html` (five anchor-based edits: registered `wsPaper` in the `screens` array, added the `#wsPaper` section markup after `#wsRef`, added the `.wsp-*` CSS block after `.lecc-reset`, added `wspState`/`wspExercises`/`wspKeyHtml`/`openWsPaper`/`wspRender`/`wspOpen`/`wspPick`/`wspWhy`/`wspRepeat`/`wspGenExtra` before `startWsCategory`, added a 📄 entry button in `openWsCategory`), then re-baked into `pebble-app.html`.
**What changed:** New "modo papel" screen per Veo worksheet category: a word key box at top (derived from the category's exercise options + desc), numbered questions with tap-to-fill blanks (tapping shows 4 option chips, picking locks the answer, wrong answers show the correct fix inline), a "¿por qué?" toggle per answered question, a running score once all questions are answered, and Repetir / Más preguntas action buttons. `wspExercises(cat)` already reads `window.__wsExtra[cat.id]` (Task 1's store) so Task 3 only needs to wire the message listener; `wspGenExtra` is a stub ("Disponible pronto.") for Task 3 to implement.
**Verification:** `checkblock.py wspRender` -> `JS SYNTAX OK`; `rebake.py "wspRender"` -> `re-bake OK marker-present`; `checkbabel.js "PEBBLE_BUILD"` -> `BABEL SYNTAX OK`. Functional check via Chrome MCP against `python3 -m http.server 8123`: `openWsPaper(1)` showed the screen with 8 questions, tapping a blank showed 4 option chips, picking filled the blank, no console errors. 375px viewport check (window resize was clamped by the OS/window manager, so used a same-origin 375x800 iframe instead): `scrollWidth === innerWidth` (no horizontal overflow), screenshot confirmed the key box, numbered list, and option chips render fully inside the 375px viewport. Full report: `.superpowers/sdd/hoja-task-2-report.md`.
**Committed:** YES, see commit hash in that commit. **Pushed:** NO (not requested, per rule).
**Notes for next session:** Task 2 done. Task 3 will wire the `wsExtraData` postMessage listener inside the iframe (setting `window.__wsExtra`), post generated exercises back to the parent as `{type:'wsExtra', catId, exercises}`, and implement `wspGenExtra()` (currently a stub) to call the AI and append to the paper worksheet.

## 2026-07-09, Hoja de Trabajo Task 3: AI-generated extra questions + persistence round-trip (BUILD 21)
**Working on:** Task 3 of the Hoja de Trabajo "modo papel" plan, replacing the `wspGenExtra` stub with real Claude generation and wiring the `wsExtraData` message into the Veo iframe, per `.superpowers/sdd/task-3-brief.md`.
**Files changed:** `veo-y-digo-source.html` (two anchor-based edits per the brief: inserted the `wsExtraData` message listener after the `esLessonsData` listener; replaced the `wspGenExtra` stub with the full async Claude call, matching `leccGenerate`'s exact fetch headers, model `claude-sonnet-4-6`, max_tokens 1500), `pebble-app.html` (BUILD 20 -> 21, note "hoja de trabajo: modo papel + preguntas nuevas con IA"), then re-baked.
**Bug found and fixed (deviation from brief):** the brief's verbatim listener code guarded the re-render with `typeof currentScreen !== 'undefined' && currentScreen === 'wsPaper'`. Testing showed this never fires: `openWsPaper` (from Task 2) calls `show('wsPaper')` directly, never `nav('wsPaper')`, and the whole ws* screen subtree (`openWsCategory`, `openWsRef`, `openWsPaper`, and their back buttons) bypasses the app's global `currentScreen`/`nav()` system entirely, so `currentScreen` never becomes `'wsPaper'`. First functional pass confirmed the failure: `window.__wsExtra` got set correctly but `wspRender()` never fired (rows stayed 8, divider false). Fixed by checking the actual DOM signal `show()` uses instead: `document.getElementById('wsPaper').classList.contains('active')`. Re-tested and confirmed correct (rows 9, divider true). Re-ran all three gates after the fix.
**Credit rule:** compliant. Generation is called only from the explicit "Más preguntas" tap (`wspGenExtra`), no auto-generation, no regen on load. Shape guard filters to items where `after` is a string, `options` is an array length >= 2, `answer` is a string and one of `options`; zero valid items throws before any cache write or postMessage.
**Verification:**
- `python3 scratchpad/checkblock.py veo-y-digo-source.html "wspGenExtra"` -> `JS SYNTAX OK`
- `python3 scratchpad/rebake.py "wspGenExtra"` -> `re-bake OK marker-present`
- `node scratchpad/checkbabel.js pebble-app.html "PEBBLE_BUILD"` -> `BABEL SYNTAX OK`
- Functional, via Chrome MCP against `python3 -m http.server 8124` serving `veo-y-digo-source.html` directly (no real paid API calls made): no-key path (`window.__claudeKey=''`, `wspGenExtra()`) showed the key-hint message, no network call. `wsExtraData` receive path (`postMessage({type:'wsExtraData', extra:{tu:[...]}})`) showed 9 rows with the "Preguntas nuevas" divider present (after the currentScreen fix; failed before it). Stubbed `window.fetch` to return a canned valid `{"exercises":[...]}` and spied on `window.parent.postMessage`: valid items were filtered/mapped into `window.__wsExtra[catId]` and a `{type:'wsExtra', catId, exercises}` message was posted to parent, with fetch headers confirmed exactly matching `leccGenerate`. Second stub returned an item with `answer` not in `options`: confirmed no cache write (`window.__wsExtra[catId]` stayed undefined), no postMessage, and the Spanish error message rendered. No console errors across all runs.
- Did not run Step 6 (pebble-app.html full round-trip); the brief marks it optional ("if convenient") since Task 1 already verified the parent-side plumbing.
**Committed:** YES, dce2b07. **Pushed:** NO, awaiting Gabby's OK + on-device test.
**Notes for next session:** on-device test plan is in `.superpowers/sdd/task-3-brief.md` (bottom section): Settings should show BUILD 21; Español -> Hoja de Trabajo -> 📄 on a category -> "✦ Más preguntas" with a real key set should add ~6 new questions under "Preguntas nuevas"; close and reopen the app, return to the sheet, confirm the generated questions persist with no new API call. Full task report: `.superpowers/sdd/hoja-task-3-report.md`.

## 2026-07-09, Hoja de Trabajo review fixes: double-tap guard + persistence round-trip
**Working on:** two review fixes flagged on the Task 3 "Mas preguntas" work. FIX 1 (code): a double-spend risk where `wspGenExtra` disabled `#wspGenBtn` imperatively with no state flag, so a `wspRender()` triggered mid-fetch by tapping a blank/option/why on an already-loaded question would rebuild the button enabled again, letting a second tap fire a second paid API call. FIX 2 (verification only): run the full `pebble-app.html` persistence round-trip that Task 3 had skipped as optional.
**Files changed:** `veo-y-digo-source.html` (added `wspState.generating` guard at the top of `wspGenExtra`, set true plus an immediate `wspRender()` before the fetch, set false after the shared try/catch before the final `wspRender()`; removed the now-redundant imperative `btn.disabled`/`btn.textContent` lines; made the `#wspGenBtn` markup in `wspRender` conditional on `wspState.generating`, disabled with "Creando preguntas…" while true), `pebble-app.html` (re-baked, no BUILD bump, not required by this fix).
**Verification:**
- `python3 scratchpad/checkblock.py veo-y-digo-source.html` -> `JS SYNTAX OK`
- `python3 scratchpad/rebake.py "wspGenExtra"` -> `re-bake OK marker-present`
- `node scratchpad/checkbabel.js pebble-app.html "PEBBLE_BUILD"` -> `BABEL SYNTAX OK`
- Functional guard test via Chrome MCP against `python3 -m http.server 8321` serving `veo-y-digo-source.html`: stubbed `window.fetch` with a 4s delay, started `wspGenExtra()`, then in the same synchronous tick called `wspRender()` (simulating a tap-triggered re-render) and `wspGenExtra()` again (simulating a second tap) before yielding to the event loop. Mid-flight: `generating:true`, button `disabled:true`, no `onclick`, label "Creando preguntas…", `fetchCallCount` stayed at 1 after the second call (no-op confirmed). After the stub resolved: `fetchCallCount:1` total, `generating:false`, button re-enabled with `onclick` restored and original label. First test attempt (two separate tool round-trips instead of one synchronous tick) gave a false read due to real wall-clock delay between calls exceeding the stub delay; caught and corrected before reporting a result.
- FIX 2 round-trip via Chrome MCP against `python3 -m http.server 8321` serving `pebble-app.html`: posted `{type:'wsExtra', catId:'tu', exercises:[...]}` from the top window, confirmed `JSON.parse(localStorage.getItem('pebble-data')).wsExtra.tu` landed the item (length 1); re-dispatched the identical message and confirmed no duplicate (length stayed 1, dedup by `before+after` confirmed). No console errors. No bug found, verification only.
**Data safety:** localhost:8321 was a fresh port/origin never used before this session; `localStorage['pebble-data']` there was the app's default-initialized state (empty profile name, empty journal, all sections default-shaped), not Gabby's real data. Snapshotted the raw string to a page-scoped JS variable before any write anyway, restored it byte-for-byte after the test, and verified the restore before closing the tab.
**Committed:** YES, `40e55cd` (FIX 1 code only; FIX 2 is verification, no code changed). **Pushed:** NO, per rule.
**Notes for next session:** no BUILD number bump in this session (not required for this fix; Task 3's BUILD 21 stands). Full fix report appended to `.superpowers/sdd/hoja-task-3-report.md`.

## 2026-07-08 - Hoja de Trabajo "modo papel" COMPLETE (BUILD 21)
**Working on:** Full feature built subagent-driven from the plan at docs/superpowers/plans/2026-07-08-hoja-papel.md (spec 2026-07-08-hoja-papel-design.md). A second, paper-style worksheet mode per category in the Veo Hoja de Trabajo, with AI-generated extra questions.
**What shipped (Tasks 1-3, each reviewed clean):**
- Task 1 (b325e1c): parent React store data.wsExtra + WS_EXTRA_ADD reducer (dedup by before+after, filters malformed) + postMessage plumbing (wsExtraData out on load/change, wsExtra in) mirroring the esLessons pattern.
- Task 2 (ded95e8): the wsPaper screen in the Veo source. A 📄 button per category opens a paper page: word key on top (from options + desc, links to the wsRefs guide), all questions numbered, tap a blank -> option chips -> pick locks it green/red with the correct word + a ¿por qué? toggle, score line when all filled, ↺ Repetir. All AI/text values escaped via esc(); option picks use index-based onclicks (no apostrophe injection).
- Task 3 (dce2b07) + fix (40e55cd): "✦ Más preguntas" calls claude-sonnet-4-6 on explicit tap only, requests an object {"exercises":[...]} (matches leccParseJson's outer-brace slice), shape-guards (answer must be in options) before appending to window.__wsExtra and posting wsExtra to the parent for persistence. Fixed a plan bug (the currentScreen guard was dead code; now checks the wsPaper active class) and added a wspState.generating flag so a re-render mid-fetch cannot re-enable the button and double-charge.
**CREDIT RULE honored:** generation only on the explicit tap; cached questions persist in data.wsExtra and reload for free; no auto-generation anywhere.
**Verification:** per-task syntax gates (checkblock/checkbabel) + rebake round-trips + Chrome-MCP functional tests (no-key path, wsExtraData receive, stubbed-fetch generation with postMessage spies, shape-guard rejection, single-tick double-tap race = fetchCallCount 1, full parent round-trip localStorage.wsExtra + dedup with Gabby's data restored). Final whole-branch review (opus): READY TO MERGE - parent<->iframe contract consistent, credit rule holds, escaping complete. PEBBLE_BUILD 20 -> 21, bake in sync.
**Committed:** YES (b325e1c, ded95e8, dce2b07, 40e55cd + docs). **Pushed:** NO - awaiting Gabby's OK to deploy.
**Known minors (non-blocking, deferred):** wspState.generating not explicitly initialized (falsy = correct); dead blank field on generated items; word-key leaves the 4th option word unglossed in the tu category (guide covers it); mid-fetch category-switch can show a stale message or clear the other category's guard (needs two overlapping cross-category generations; the single-category guard is solid).
**On-device test plan (BUILD 21):** Español -> Hoja de Trabajo -> 📄 on Tú/Te/Tu: key box + 8 numbered questions; tap a blank -> chips -> green/red + ¿por qué?; finish -> score; ↺ Repetir clears; ✦ Más preguntas (key set) -> ~6 new questions under "Preguntas nuevas"; fully close + reopen the app, return to the sheet -> generated questions still there with NO API call.

## 2026-07-09 - Fix: conversation flashcard audio spoke English, not Spanish (BUILD 22)
**Bug (Gabby):** On flashcards made from the Spanish chat, the 🔊 "escuchar" button played the ENGLISH word, not the Spanish. (Her words: the mic plays the English, which is useless since she already knows how to pronounce the English.) The card orientation itself was fine; only the audio was wrong.
**Root cause (verified in-browser, not guessed):** `sayCard()` speaks `c.word`. Built-in picture cards store the Spanish in `word` (+ English in `en`), so they speak Spanish correctly. But the session-card mapping (applyEsFlashSets, `themeDecks[label]=...{ front:c.es, word:(c.en||c.es), en:c.es }`) puts the ENGLISH in `word` and the Spanish in `front`. So `sayCard` read the English aloud for conversation cards only. Reproduced: session card spoke "beach"; picture card spoke "el sol".
**Fix:** `sayCard` now prefers `c.front` when present (that is the Spanish text on session cards); picture/story cards have no `front` and are unchanged. One-line change, no visual/orientation change (Gabby said the front/back is fine as-is). Confirmed in-browser: session card now speaks "playa", picture card still "el sol".
...
## 2026-07-09 - Flashcards item 4: create deck by subject + generar 5 mas (BUILD 25)
**Working on:** Final item of the flashcard batch. Gabby: a button + subject (e.g. "beach") -> AI builds a themed deck; she picks the size and can "generate 5 more" later. Spanish deck name, es+en cards.
**What shipped (FlashEditor, React-side):** "mazo nuevo por tema" section: subject input + size select (5/10/15/20, default 10) + "crear mazo". createDeck() asks Claude for {name, cards[]} (parseJ object), validates, makes a themed esFlash deck (ES_FLASH_NEW_SET now stores `subject`), adds es+en cards, switches to it. Subject decks show "generar 5 mas" -> generateMore() asks for 5 more avoiding existing, ADD_CARD (reducer dedups). Both key-gated + aiBusy "…" + explicit-tap only (credit rule). New decks sync into the Veo study screen via the existing esFlashSets push and are reachable via item 2's "editar lista".
**Reducer:** ES_FLASH_NEW_SET spreads `subject: action.subject || null` (additive; themed decks survive mergeConvoSets).
**Files:** pebble-app.html only, no re-bake. Spec docs/superpowers/specs/2026-07-09-flashcard-create-deck-by-subject.md. PEBBLE_BUILD 24 -> 25.
